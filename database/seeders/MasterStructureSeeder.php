...
class MasterStructureSeeder extends Seeder
{
    ...
    private function createGrupoEsplendidoApps(Enterprise $enterprise): void
    {
        ...
        Submodule::firstOrCreate(
            ['slug' => 'checador-fijo', 'module_id' => $rhAsistencia->id],
            ['name' => 'Checador Fijo', 'icon' => 'MonitorSmartphone', 'order' => 4, 'is_active' => true]
        );
    ...
    private function createSplendidFarmsApps(Enterprise $enterprise): void
    {
        ...
        Submodule::firstOrCreate(
            ['slug' => 'empaque', 'module_id' => $reportesAdmin->id],
            ['name' => 'Empaque', 'icon' => 'ClipboardList', 'order' => 3, 'is_active' => true]
        );
        $this->command->info("    → Reportes: Embarques, Productores, Empaque");
